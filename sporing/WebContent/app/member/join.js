...
const getFindHtml = function (type) {
	...
	const fnXHR = function (method, url, jsonData, formData, callback) {
		...
		const SWalertPreConfirm = function (icon, html, confirmButtonText = '확인', preConfirm) {
			...
			let btnSendCertify = document.querySelector('button[name="btnSendCertify"]');
			if (btnSendCertify)
			  btnSendCertify.addEventListener('click', function () {
			    ...
			    let receiver = user_phone.value + phone1.value + phone2.value;
			    let xhr = new XMLHttpRequest();
			    xhr.open('GET', '/users/certify/' + receiver, true);
			...
			let btnCertify = document.querySelector('button[name="btnCertify"]');
			if (btnCertify)
			  btnCertify.addEventListener('click', function () {
			    ...
			    xhr.onreadystatechange = function () {
			      if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
			        ...
			        if (res.result) {
			          alert('인증 성공');
			          document.querySelector('input[name="sessionID"]').value = res.result.sessionID;
			        } else {
